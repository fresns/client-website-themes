...
@if ($files['images'])
    @foreach($files['images'] as $image)
        @if (count($files['images']) == 1)
            ...
            @if (Route::is('fresns.post.detail'))
                ...
                <a class="position-relative image-box" style="width:auto;" data-fancybox="post-{{ $pid }}" data-src="{{ $image['imageBigUrl'] }}">
                    <img src="{{ $image['imageRatioUrl'] }}" loading="lazy" alt="{{ $image['name'] }}" class="img-fluid" style="max-height:none;">
                    @if ($image['imageLong'])
            ...
            @else
                ...
                <a class="position-relative image-box" data-fancybox="post-{{ $pid }}" data-src="{{ $image['imageBigUrl'] }}">
                    <img src="{{ $image['imageLong'] ? $image['imageSquareUrl'] : $image['imageRatioUrl'] }}" loading="lazy" alt="{{ $image['name'] }}" class="img-fluid">
                    @if ($image['imageLong'])
                        <span class="position-absolute bottom-0 end-0 badge bg-light text-dark me-3 mb-2">{{ fs_lang('contentImageLong') }}</span>
                    @endif
                </a>
        ...
        @else
            {{-- Multiple Images --}}
            <a class="position-relative image-box" data-fancybox="post-{{ $pid }}" data-src="{{ $image['imageBigUrl'] }}">
                <img src="{{ $image['imageSquareUrl'] }}" loading="lazy" alt="{{ $image['name'] }}" class="img-fluid">
                @if ($image['imageLong'])
                    <span class="position-absolute bottom-0 end-0 badge bg-light text-dark me-3 mb-2">{{ fs_lang('contentImageLong') }}</span>
                @endif
            </a>

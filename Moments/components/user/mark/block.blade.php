<form action="{{ route('fresns.api.user.mark') }}" method="post" class="float-start me-2">
    ...
    @if ($interaction['blockStatus'])
        <a class="btn btn-success btn-sm fs-mark" data-interaction-active="{{ $interaction['blockStatus'] }}" data-bi="fa-regular fa-circle-xmark" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ fs_lang('cancel') }}">
            ...
            @if (fs_config('user_blocker_count') && $count)
    ...
    @else
        ...
        <div class="collapse" id="collapse-{{ $uid }}">
            <div class="card card-body my-1">
                ...
                <a class="btn btn-danger btn-sm fs-mark" role="button" data-id="fs-mark-block-{{ $uid }}" data-collapse-id="collapse-{{ $uid }}" data-bi="fa-solid fa-circle-xmark" data-name="{{ $interaction['blockName'] }}" href="javascript:void(0)">{{ fs_lang('confirm') }}</a>
            </div>
        </div>
    @endif
</form>

    ...
    @else
        ...
        <div class="collapse" id="collapse-{{ $hid }}">
            <div class="card card-body my-1">
                ...
                <a class="btn btn-danger btn-sm fs-mark" role="button" data-id="fs-mark-block-{{ $hid }}" data-collapse-id="collapse-{{ $hid }}" data-bi="fa-solid fa-circle-xmark" data-name="{{ $interaction['blockName'] }}" href="javascript:void(0)">{{ fs_lang('confirm') }}</a>
            </div>
        </div>
    @endif
</form>

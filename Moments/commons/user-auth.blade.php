                    ...
                    <div class="row">
                        @foreach(fs_account('detail.users') as $item)
                            <div class="col-6 col-md-4 d-flex flex-column align-items-center">
                                ...
                                <form action="{{ route('fresns.api.post', ['path' => '/api/fresns/v1/user/auth-token']) }}" id="uid-{{ $item['uid'] }}" method="post">
                                    @csrf
    ...
    <div class="modal fade" id="userPwdLogin" aria-hidden="true" aria-labelledby="userPwdLoginLabel" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content" id="user-password-auth">
                <form action="{{ route('fresns.api.post', ['path' => '/api/fresns/v1/user/auth-token']) }}" method="post" onsubmit="var passwordInput = document.querySelector('#user-password-auth > form > div.modal-body > div.input-group > input'); passwordInput.value = Base64.encode(passwordInput.value)">
                    <div class="modal-header">

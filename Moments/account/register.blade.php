...
@section('title', fs_config('menu_account_register'))
    ...
    <div class="p-3 ms-lg-5 ps-lg-5 mb-5" style="max-width:580px;">
        ...
        @if (fs_config('site_public_status'))
            ...
            @if (fs_config('account_connect_services'))
                ...
                @if (fs_config('site_email_register') || fs_config('site_phone_register'))
            ...
            @if (fs_config('site_email_register') || fs_config('site_phone_register'))
                <form class="py-3" id="accordionCodeAccount" method="post" novalidate action="{{ route("fresns.api.account.register") }}">
                    ...
                    @if (fs_config('site_email_register') && fs_config('site_phone_register'))
                        <div class="input-group mb-3 mt-2">
                            ...
                            <div class="form-control">
                                {{-- Email --}}
                                @if (fs_config('site_email_register'))
                                ...
                                {{-- Phone Number --}}
                                @if (fs_config('site_phone_register'))
                                    <div class="form-check form-check-inline">
                    ...
                    @elseif (fs_config('site_email_register'))
                        <input type="radio" name="type" value="email" class="d-none" checked>
                    @elseif (fs_config('site_phone_register'))
                    ...
                    <div>
                        ...
                        {{-- Phone Number --}}
                        @if (fs_config('site_phone_register'))
                            <div class="collapse code_account_phone @if (old('type') == 'phone' || ! fs_config('site_email_register')) show @endif" aria-labelledby="code_account_phone" data-bs-parent="#accordionCodeAccount">
                                <div class="input-group mb-3">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <input type="password" class="form-control" minlength="{{ fs_config('password_length') }}" maxlength="32" name="password" required>
                    </div>

                    {{-- Enter Your Password Again --}}
                    <div class="input-group">
                        <span class="input-group-text">{{ fs_lang('passwordAgain') }}</span>
                        <input type="password" class="form-control" minlength="{{ fs_config('password_length') }}" maxlength="32" name="password_confirmation" required>
                    </div>
                    ...
                    <label class="form-label fs-6 mt-2">
                        ...
                        @if (fs_config('account_cookies_status'))
                            <a class="badge rounded-pill bg-success link-light text-decoration-none" data-bs-toggle="modal" href="#cookiesModal">{{ fs_lang('accountPoliciesCookies') }}</a>
                        @endif
                    </label>
        ...
        @else
            ...
            <div class="alert alert-danger" role="alert">
                {{ fs_lang('accountRegisterCloseDesc') }}
            </div>
            ...
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        {!! fs_config('account_terms') ? Str::markdown(fs_config('account_terms')) : '' !!}
                    </div>
        ...
        <div class="modal fade" id="privacyModal" tabindex="-1" aria-labelledby="privacyModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        {!! fs_config('account_privacy') ? Str::markdown(fs_config('account_privacy')) : '' !!}
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ fs_lang('close') }}</button>
                    </div>
    ...
    @if (fs_config('account_cookies_status'))
        <div class="modal fade" id="cookiesModal" tabindex="-1" aria-labelledby="cookiesModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="cookiesModalLabel">{{ fs_lang('accountPoliciesCookies') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        {!! fs_config('account_cookies') ? Str::markdown(fs_config('account_cookies')) : '' !!}
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">{{ fs_lang('close') }}</button>
                    </div>

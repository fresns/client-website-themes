            ...
            <div class="toast-body">
                ...
                @if (session('code') == 36104)
                    <div class="mt-2 pt-2 border-top">
                        <kbd>{{ fs_config('menu_account_settings') }}->{{ fs_lang('settingAccount') }}</kbd>
                ...
                @if (session('code') == 38200)
                    <div class="mt-2 pt-2 border-top">
                        <a class="btn btn-primary btn-sm" href="{{ fs_route(route('fresns.me.drafts', ['type' => 'posts'])) }}" role="button">{{ fs_lang('view') }}</a>
                    </div>
                @endif
            </div>
        </div>

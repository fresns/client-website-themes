...
function accountVerification(obj)
{
    ...
    $.ajax({
        ...
        success: function(res){
            ...
            let html = '';
            $(obj).parent().siblings().each(function (k,v){
...
;(function($){
    ...
    $(".fresns-content").atwho({
        ...
        callbacks: {
            remoteFilter: function (query, callback) {
                if (query) {
                    $.get("/api/engine/input-tips", {"type": 'user', "key": query}, function (data) {
                        callback(data)
                    }, 'json')
                }
            }
        ...
        callbacks: {
            remoteFilter: function (query, callback) {
                if (query) {
                    $.get("/api/engine/input-tips", {"type": 'hashtag', "key": query}, function (data) {
                        callback(data);
                    }, 'json')
                }
            }
            ...
            success: function (res) {
                ...
                if (fresnsReply.prev().find('.cm-count').length === 1) {
                    if (res.code === 0) {
                        ...
                        fresnsReply.prev().find('.cm-count').text(parseInt(oldCount) + 1)
                        form[0].reset()
                        showReply(fresnsReply)
                    }
                    btn.prop('disabled', false);
            ...
            function (e){
                ...
                if (bi) {
                    ...
                    if (interactiveActive) {
                        ...
                        obj.hasClass('btn') ? obj.removeClass('btn-success').addClass('btn-outline-success') : obj.removeClass('text-success');
                    } else {
                        obj.find("i").addClass("bi " + bi);
                        obj.hasClass('btn') ? obj.addClass('btn-success').removeClass('btn-outline-success') : obj.addClass('text-success');
                    }
                }
                ...
                if (interactiveType == 'like') {
                    ...
                    if (likeOrDislikeObjBi) {
                        ...
                        const newBi = fillPosition != 0 ? likeOrDislikeObjBi.slice(0, fillPosition) : likeOrDislikeObjBi.slice(0)
                        ...
                        if (likeOrDislikeObjIconActive && likeOrDislikeObjInteractiveActivate) {
                            likeOrDislikeObj.find('img').attr('src', likeOrDislikeObjInteractiveActivate == 0 ? likeOrDislikeObjIconActive : likeOrDislikeObjIcon)
                            likeOrDislikeObj.hasClass('btn') ? likeOrDislikeObj.removeClass('btn-active').addClass('btn-outline-success') : likeOrDislikeObj.removeClass('text-success');
                        }
                    }
    ...
    $(document).on('click', '.web-request-link', function (e) {
        ...
        $(this).prepend('<span class="spinner-border spinner-border-sm mg-r-5" role="status" aria-hidden="true"></span> ')
        ...
        switch (type) {
            case "select":
                ...
                <div class="input-group has-validation">
                    <label class="input-group-text">`+ lable +`</label>
            ...
            default:
                if (name === "editPhone" && ! value) {
                    ...
                    html += `
                        ...
                        <input type="text" class="form-control w-50" name="` + name + `" value="" required>
                        <input type="hidden" name="codeType" value="sms">
                    </div>
                    <div class="input-group has-validation">
                        <span class="input-group-text">${fs_lang('verifyCode')}</span>
                        <input type="text" class="form-control" name="newVerifyCode" required>
                        <button data-type="sms" data-use-type="3" data-template-id="4" data-action="/api/engine/send-verify-code" onclick="sendVerifyCode(this)"  class="btn btn-outline-secondary send-verify-code" type="button">${fs_lang('sendVerifyCode')}</button>
                    </div>`
                    ...
                    <div class="input-group has-validation">
                        <span class="input-group-text">${fs_lang('verifyCode')}</span>
                        <input type="text" class="form-control" name="newVerifyCode">
                        <button data-type="email" data-use-type="3" data-template-id="4" data-action="/api/engine/send-verify-code" onclick="sendVerifyCode(this)" class="btn btn-outline-secondary send-verify-code" type="button">${fs_lang('sendVerifyCode')}</button>
                    </div>`
                    ...
                    <div class="input-group has-validation mb-3">
                        <span class="input-group-text">` + lable + `</span>
                        <input class="form-control" type="text" placeholder="` + value + `" value="` + value + `" disabled>
                        <button data-type="sms" data-use-type="4" data-template-id="4" data-action="/api/engine/send-verify-code" onclick="sendVerifyCode(this)"  class="btn btn-outline-secondary send-verify-code" type="button">${fs_lang('sendVerifyCode')}</button>
                    </div>
                    <div class="input-group has-validation mb-3">
                        <span class="input-group-text">${fs_lang('verifyCode')}</span>
                        <input type="text" class="form-control" name="verifyCode">
                        <button class="btn btn-outline-secondary" data-code-type="sms" data-action="/api/engine/account/verify-identity" type="button" onclick="accountVerification(this)">${fs_lang('check')}</button>
                    </div>
                    <div class="input-group has-validation mb-3 d-none">
                        <span class="input-group-text">${fs_lang('newPhone')}</span>
                        <select class="form-select" name="editCountryCode">
                            <option disabled>Country Calling Codes</option>`;
                    $(smsCodes).each(function (k, v) {
                        let selected = (v == defaultSmsCode) ? "selected" : "";
                    ...
                    html += `</select>
                        <input type="text" class="form-control w-50" required name="` + name + `" value="">
                    </div>
                    <div class="input-group has-validation d-none">
                        <span class="input-group-text">${fs_lang('newVerifyCode')}</span>
                        <input type="text" class="form-control" name="newVerifyCode">
                        <input type="hidden" name="codeType" value="sms">
                        <button data-type="sms" data-use-type="1" data-template-id="3" data-action="/api/engine/send-verify-code" onclick="sendVerifyCode(this)"  class="btn btn-outline-secondary send-verify-code" type="button">${fs_lang('sendVerifyCode')}</button>
                    </div>`
                    ...
                    <div class="input-group mb-3">
                        <span class="input-group-text">` + lable + `</span>
                        <input class="form-control" type="text" placeholder="` + value + `" value="` + value + `" disabled>
                        <button data-type="email" data-use-type="4" data-template-id="4" data-action="/api/engine/send-verify-code" onclick="sendVerifyCode(this)"  class="btn btn-outline-secondary send-verify-code" type="button">${fs_lang('sendVerifyCode')}</button>
                    </div>
                    <div class="input-group has-validation mb-3">
                        <span class="input-group-text">${fs_lang('verifyCode')}</span>
                        <input type="text" class="form-control" name="verifyCode">
                        <button class="btn btn-outline-secondary" required data-code-type="email" data-action="/api/engine/account/verify-identity" type="button" onclick="accountVerification(this)">${fs_lang('check')}</button>
                    </div>
                    <div class="input-group has-validation mb-3 d-none">
                        <span class="input-group-text">${fs_lang('newEmail')}</span>
                    ...
                    <div class="input-group d-none">
                        ...
                        <input type="text" class="form-control" name="newVerifyCode">
                        <input type="hidden" name="codeType" value="email">
                        <button data-type="email" data-use-type="1" data-template-id="3" data-action="/api/engine/send-verify-code" onclick="sendVerifyCode(this)"  class="btn btn-outline-secondary send-verify-code" type="button">${fs_lang('sendVerifyCode')}</button>
                    </div>`
                } else if(name === "editPassword" || name === "editWalletPassword") {
                    html =`
                    <div class="input-group mb-3 mt-2">
                        <span class="input-group-text">${fs_lang('settingType')}</span>
                        <div class="form-control">`
                    if (value) {
                        html += `
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="`+ name +`_mode" id="password_to_edit" value="password_to_`+ name +`" data-bs-toggle="collapse" data-bs-target="#password_to_edit:not(.show)" aria-expanded="true" checked>
                    ...
                    html += `
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="`+ name +`_mode" id="phone_to_edit" value="phone_to_`+ name +`" data-bs-toggle="collapse" data-bs-target="#phone_to_edit:not(.show)" aria-expanded="false">
                                <label class="form-check-label" for="phone_to_edit">${fs_lang('smsVerifyCode')}</label>
                            </div>
                        </div>
                    </div>
                    <div id="edit_password_mode">
                        <div class="collapse ${ value ? "show" : '' }" id="password_to_edit" aria-labelledby="password_to_edit" data-bs-parent="#edit_password_mode">
                            <div class="input-group mb-3">
                                <span class="input-group-text">${fs_lang('passwordCurrent')}</span>
                                <input type="hidden" class="form-control" name="edit_type" value="` + name + `">
                                <input type="password" class="form-control" name="now_` + name + `">
                            </div>
                        </div>
                        <div class="collapse ${ !value ? "show" : ""}" id="email_to_edit" aria-labelledby="email_to_edit" data-bs-parent="#edit_password_mode">
                            <div class="input-group mb-3">
                                <span class="input-group-text">${fs_lang('email')}</span>
                                <input class="form-control" type="text" placeholder="` + email + `" value="` + email + `" disabled>
                            </div>
                            <div class="input-group mb-3">
                                <span class="input-group-text">${fs_lang('verifyCode')}</span>
                                <input type="text" class="form-control" name="email_verifyCode">
                                <button data-type="email" data-use-type="4" data-template-id="5" data-action="/api/engine/send-verify-code" onclick="sendVerifyCode(this)"  class="btn btn-outline-secondary send-verify-code" type="button">${fs_lang('sendVerifyCode')}</button>
                            </div>
                        </div>
                        <div class="collapse" id="phone_to_edit" aria-labelledby="phone_to_edit" data-bs-parent="#edit_password_mode">
                            <div class="input-group mb-3">
                                ...
                                <input class="form-control" type="text" placeholder="` + phone + `" value="` + phone + `" disabled>
                            </div>
                            <div class="input-group mb-3">
                                <span class="input-group-text">${fs_lang('verifyCode')}</span>
                                <input type="text" class="form-control" name="phone_verifyCode">
                                <button data-type="sms" data-use-type="4" data-template-id="5" data-action="/api/engine/send-verify-code" onclick="sendVerifyCode(this)"  class="btn btn-outline-secondary send-verify-code" type="button">${fs_lang('sendVerifyCode')}</button>
                            </div>
                        </div>
                        <div class="input-group mb-3">
                            <span class="input-group-text">${fs_lang('passwordNew')}</span>
                            <input type="password" class="form-control" name="new_` + name + `">
                        </div>
                        <div class="input-group mb-3">
                            <span class="input-group-text">${fs_lang('passwordAgain')}</span>
                            <input type="password" class="form-control" name="new_` + name + `_confirmation">
                        </div>
                    </div>`
                } else {
                    html =`
    ...
    $("#editModal.user-edit form button[type='submit']").on('click', function (e){
        ...
        obj.parent().prev().find('input, textarea, select').each(function(k, v){
            if (!$(v).val() && !$(v).is(':disabled') && (!$(v).parent().parent().hasClass("collapse") || $(v).parent().parent().hasClass( "show"))) {
                ...
                if (!$(v).hasClass('is-invalid')) {
                    ...
                    if ($(v).parent().hasClass('d-none') && $(v).parent().parent().find('input[name="verifyCode"]').length) {
                        form.find('.modal-body .invalid-feedback').length ? form.find('.modal-body .invalid-feedback').text(fs_lang('settingCheckError')) : form.find('.modal-body').append(`<div class="invalid-feedback d-block">${fs_lang('Please verify first')}</div>`);
                    } else if ($(v).next().length) {
                        $(v).parent().append(`<div class="invalid-feedback d-block">` + message + `</div>`)
                    } else {
        ...
        if (result['edit_type'] !== undefined) {
            ...
            if (result[`new_${result['edit_type']}_confirmation`] !== undefined) {
                result[`new_${result['edit_type']}_confirmation`] = window.btoa(result[`new_${result['edit_type']}_confirmation`]);
            }
        }
        ...
        window.buildAjaxAndSubmit(
            ...
            function (e){
                if (!targeObj.length) {
                    form.find('.modal-body').append(`<div class="invalid-feedback d-block">` + e.responseJSON.message + `</div>`);
                } else {
    ...
    $("#uploadAvatar").on('change', function (e) {
        ...
        obj.prop('disabled', true);
        obj.prev().prepend('<span class="spinner-border spinner-border-sm mg-r-5" role="status" aria-hidden="true"></span> ')
...
$(document).ready(function () {
	...
	$('.markdown a:not(:has(img)),.markdown a').hover(function() {
